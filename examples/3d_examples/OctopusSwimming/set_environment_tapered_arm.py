...
class ArmEnvironment:
    ...
    def save_data(self):
        ...
        # Save octopus data
        n_elem_arm = self.arm_rod_list[0].n_elems
        n_arms = len(self.arm_rod_list)
        time = np.array(self.post_processing_dict_list[0]["time"])
        arm_position_history = np.zeros((n_arms, time.shape[0], 3, n_elem_arm + 1))
        arm_velocity_history = np.zeros((n_arms, time.shape[0], 3, n_elem_arm + 1))
        arm_radius_history = np.zeros((n_arms, time.shape[0], n_elem_arm))

        for i in range(n_arms):
            arm_position_history[i, :, :, :] = np.array(
        ...
        np.savez(
            ...
            body_position_history=body_position_history,
            body_velocity_history=body_velocity_history,
            body_radius_history=body_radius_history,
            head_position_history=head_position_history,
