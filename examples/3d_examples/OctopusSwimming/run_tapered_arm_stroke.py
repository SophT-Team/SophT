...
def tapered_arm_and_cylinder_flow_coupling(
    ...
    coupling_stiffness=-2e2,
    coupling_damping=-1e-1,
    num_threads=4,
    precision="single",
    save_data=True,
):
    ...
    octopus_head_data = np.load("octopus_head_data.npz")
    slenderness_ratio_head = octopus_head_data["slenderness_ratio"]
    non_dimensional_radius = octopus_head_data["non_dimensional_radius"]

    octopus_head_length = (
        2 * base_radius * non_dimensional_radius[0]
    ) * slenderness_ratio_head
    octopus_head_n_elems = int(n_elems * octopus_head_length / base_length)

    from scipy.interpolate import interp1d

    radius_function = interp1d(
        octopus_head_data["non_dimensional_lengths"],
        octopus_head_data["non_dimensional_radius"],
        kind="cubic",
    )
    non_dimensional_lengths = np.linspace(0, 1, octopus_head_n_elems + 1)
    non_dimensional_element_lengths = 0.5 * (
        non_dimensional_lengths[1:] + non_dimensional_lengths[:-1]
    )
    octopus_head_radius = base_radius * radius_function(non_dimensional_element_lengths)
    ...
    env.reset(youngs_modulus, rho_s, rod_list, arm_rod_list)
...
if __name__ == "__main__":
    ...
    def simulate_parallelised_octopus_arm(num_threads, nz):
        ...
        exp_activation_period = 2.533 * 3
        exp_activation_level_max = 0.2
