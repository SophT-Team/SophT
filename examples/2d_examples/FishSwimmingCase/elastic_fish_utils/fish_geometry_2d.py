...
def create_fish_geometry(rest_lengths: np.ndarray) -> tuple[np.ndarray, float]:
    """
    Computes the width and height of a fish given the rest lengths of
    its constituent elements.

    Parameters
    ----------
    rest_lengths : np.ndarray
        1D array containing the rest lengths of each element in the fish.

    Returns
    -------
    tuple
        A tuple containing two elements:
        - An np.ndarray of shape (n_elements,), where n_elements is the
        number of elements in the fish. This array contains the width of the fish
        at each element.
        - A float representing the height of the fish.
