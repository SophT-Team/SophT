...
def flow_past_constant_temperature_cylinder_case(
    ...
    """
    This example considers a simple flow past constant temperature cylinder using immersed
    boundary forcing.
    """
    grid_dim = 2
    ...
    while thermal_sim.time < final_time:
        ...
        dt_flow = flow_sim.compute_stable_timestep()
        dt_thermal = thermal_sim.compute_stable_timestep()

        thermal_time_step = int(dt_flow / min(dt_flow, dt_thermal))
        local_thermal_dt = dt_flow / thermal_time_step
        thermal_sim.time = flow_sim.time
        for i in range(thermal_time_step):
            cylinder_thermal_interactor.time_step(dt=local_thermal_dt)
            cylinder_thermal_interactor()
            thermal_sim.time_step(dt=local_thermal_dt)

        # # compute flow forcing and timestep forcing
        cylinder_flow_interactor.time_step(dt=dt_flow)
        cylinder_flow_interactor()

        # timestep the flow
        flow_sim.time_step(dt=dt_flow, free_stream_velocity=velocity_free_stream)
...
if __name__ == "__main__":
    ...
    def simulate_flow_past_constant_temperature_cylinder_case(
        ...
        flow_past_constant_temperature_cylinder_case(
    ...
    simulate_flow_past_constant_temperature_cylinder_case()
