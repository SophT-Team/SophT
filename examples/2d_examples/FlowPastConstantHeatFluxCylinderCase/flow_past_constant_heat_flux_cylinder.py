...
def flow_past_constant_heat_flux_cylinder_case(
    ...
    """
    This example considers a simple flow past constant temperature
    cylinder using immersed boundary forcing.
    """
    grid_dim = 2
    ...
    x_cm = 0.5 * x_range  # 10.5 * cyl_radius  # 0.2 * x_range #2.5 * cyl_radius
    y_cm = 0.5 * y_range
    ...
    while thermal_sim.time < final_time:
        ...
        if foto_timer >= foto_timer_limit or foto_timer == 0:
            ...
            ax_thermal.scatter(
                isoflux_cylinder_thermal_interactor.forcing_grid.position_field[
                    x_axis_idx
                ],
                isoflux_cylinder_thermal_interactor.forcing_grid.position_field[
                    y_axis_idx
            ...
            print(
                ...
                f"thermal grid deviation L2 error: {isoflux_cylinder_thermal_interactor.get_grid_deviation_error_l2_norm():.6f}, "
                f"temperature mismatch : {np.sum(np.fabs(isoflux_cylinder_thermal_interactor.lag_grid_velocity_mismatch_field)):.6f}, "
        ...
        if data_timer >= data_timer_limit or data_timer == 0:
            ...
            temperature_wall = (
                isoflux_cylinder_thermal_interactor.forcing_grid.velocity_field
            )

            dT_dn = (
                (isoflux_cylinder_thermal_interactor.lag_grid_forcing_field)
                / thermal_diffusivity
                / thermal_sim.dx
            )

            Nu_local = (
                dT_dn / (temperature_wall - 0) * (2 * cyl_radius)
            )  # / lag_grid_spacing

            nusslet = np.sum(np.fabs(Nu_local) * thermal_sim.dx) / (
                2 * np.pi * cyl_radius * 1.0
            )
        ...
        thermal_sim.time = flow_sim.time
...
if __name__ == "__main__":
    ...
    @click.option(
        "--sim_grid_size_x", default=768, help="Number of grid points in x direction."
    )
    @click.option(
        "--nondim_final_time",
        default=100.0,
    ...
    @click.option("--reynolds", default=20.0, help="Reynolds number.")
    @click.option("--prandtl", default=0.7, help="Prandtl number.")
    @click.option("--heat_flux", default=-1.0, help="Cylinder heat flux.")
    def simulate_flow_past_constant_heated_flux_cylinder_case(
        num_threads: int,
        sim_grid_size_x: int,
        nondim_final_time: float,
        reynolds: float,
        ...
        click.echo(f"Cylinder heat flux: {heat_flux}")

        flow_past_constant_heat_flux_cylinder_case(
    ...
    simulate_flow_past_constant_heated_flux_cylinder_case()
