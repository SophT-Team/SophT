...
def test_local_eulerian_grid_support_of_lagrangian_grid_2d(n_values, precision):
    ...
    # generate reference manufactured solution for grid [0, 1]^2
    x = np.linspace(
...
@pytest.mark.parametrize("precision", ["single", "double"])
@pytest.mark.parametrize("n_values", [16])
def test_eulerian_to_lagrangian_grid_interpolation_kernel_2d(n_values, precision):
    real_t = get_real_t(precision)
    grid_dim = 2
    eul_grid_size = n_values
    interp_kernel_width = 2
    ...
    eul_grid_field = np.ones((eul_grid_size, eul_grid_size), dtype=real_t)
...
@pytest.mark.parametrize("precision", ["single", "double"])
@pytest.mark.parametrize("n_values", [16])
def test_vector_field_eul_to_lag_grid_interpolation_kernel_2d(n_values, precision):
    real_t = get_real_t(precision)
    grid_dim = 2
    ...
    eul_grid_field = np.ones((n_components, eul_grid_size, eul_grid_size), dtype=real_t)
    # set interp weight as a series of 0 to 4 * interp_kernel_width ** 2 - 1
    ref_interp_weights = np.arange(0, 4 * interp_kernel_width**2).reshape(
        2 * interp_kernel_width, 2 * interp_kernel_width, 1
    )
...
@pytest.mark.parametrize("precision", ["single", "double"])
@pytest.mark.parametrize("n_values", [16])
def test_lagrangian_to_eulerian_grid_interpolation_kernel_2d(n_values, precision):
    real_t = get_real_t(precision)
    grid_dim = 2
    eul_grid_size = n_values
    interp_kernel_width = 2
    ...
    eul_grid_field = np.zeros((eul_grid_size, eul_grid_size), dtype=real_t)
    lagrangian_to_eulerian_grid_interpolation_kernel(
        eul_grid_field,
        ref_lag_grid_field,
        ref_interp_weights,
...
@pytest.mark.parametrize("precision", ["single", "double"])
@pytest.mark.parametrize("n_values", [16])
def test_vector_field_lag_to_eul_grid_interpolation_kernel_2d(n_values, precision):
    real_t = get_real_t(precision)
    grid_dim = 2
    n_components = grid_dim
    eul_grid_size = n_values
    ...
    np.testing.assert_allclose(
        np.sum(eul_grid_field[0]),
        ref_interpolated_field_sum_y,
        atol=get_test_tol(precision),
    )
    np.testing.assert_allclose(
        np.sum(eul_grid_field[1]),
        ref_interpolated_field_sum_x,
        atol=get_test_tol(precision),
    )
...
def test_interpolation_weights_kernel_on_nodes_2d(
    ...
    # generate manufactured solution (Y = 0, X = 1)
