...
def test_elementwise_sum_pyst_kernel_3d(n_values, precision):
    ...
    field_2 = 3 * np.ones((n_values, n_values, n_values), dtype=real_t)
...
def test_vector_field_elementwise_sum_pyst_kernel_3d(n_values, precision):
    ...
    field_2 = 3 * np.ones((3, n_values, n_values, n_values), dtype=real_t)
    sum_field = np.zeros_like(field_1)
    elementwise_sum_pyst_kernel(
        sum_field=sum_field,
        field_1=field_1,
...
def test_vector_field_set_fixed_val_pyst_kernel_3d(n_values, precision):
    ...
    fixed_vals = np.array((1, 2, 3), dtype=real_t)
    set_fixed_val_pyst_kernel(
        vector_field=field,
        fixed_vals=fixed_vals,
    )
    np.testing.assert_allclose(field[0], 1)
    np.testing.assert_allclose(field[1], 2)
    np.testing.assert_allclose(field[2], 3)
...
def test_elementwise_complex_product_pyst_kernel_3d(n_values, precision):
    ...
    field_2 = (2 + 3j) * np.ones((n_values, n_values, n_values), dtype=complex_dtype)
...
def test_vector_field_add_fixed_val_pyst_kernel_3d(n_values, precision):
    ...
    field = np.ones((3, n_values, n_values, n_values), dtype=real_t)
    fixed_vals = np.array((2, 3, 4), dtype=real_t)
    add_fixed_val_pyst_kernel(
        sum_field=field,
        vector_field=field,
        fixed_vals=fixed_vals,
    ...
    # 1 + 4 = 5
    np.testing.assert_allclose(field[2], 5)
...
def test_elementwise_cross_product_pyst_kernel_3d(n_values, precision):
    ...
    field_2 = np.zeros((dim, n_values, n_values, n_values), dtype=real_t)
    field_2 += np.array([4.0, 5.0, 6.0]).reshape(dim, 1, 1, 1)
    result_field = np.zeros_like(field_1)
    elementwise_cross_product_pyst_kernel(
    ...
    # 2 * 6 - 3 * 5 = -3
    np.testing.assert_allclose(result_field[0], -3.0)
    # 3 * 4 - 1 * 6 = 6
    np.testing.assert_allclose(result_field[1], 6.0)
    # 1 * 5 - 2 * 4 = -3
    np.testing.assert_allclose(result_field[2], -3.0)
