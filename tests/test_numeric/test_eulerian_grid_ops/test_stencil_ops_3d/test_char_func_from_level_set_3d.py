...
def char_func_from_level_set_via_sine_heaviside_reference_3d(
    char_func_field,
    level_set_field,
    blend_width,
    real_t,
...
class CharFuncFromLevelSetFuncSolution:
    def __init__(self, n_samples, precision="single"):
        ...
        self.level_set_field = np.random.randn(n_samples, n_samples, n_samples).astype(real_t)
        self.dx = real_t(0.1)
        self.blend_width = 2 * self.dx
        # later can add variations here...
        self.ref_char_func_field = np.zeros_like(self.level_set_field)
        char_func_from_level_set_via_sine_heaviside_reference_3d(
...
def test_char_func_from_level_set_via_sine_heaviside_pyst_3d(n_values, precision):
    real_t = get_real_t(precision)
