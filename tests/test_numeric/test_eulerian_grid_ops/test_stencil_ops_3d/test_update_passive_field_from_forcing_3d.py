...
def update_passive_field_from_forcing_reference(
    ...
    new_passive_field[1:-1, 1:-1, 1:-1] = (
        passive_field[1:-1, 1:-1, 1:-1] + forcing_field[1:-1, 1:-1, 1:-1] * prefactor
    )
...
class UpdatePassiveFieldFromForcingSolution:
    def __init__(self, n_samples, precision="single"):
        ...
        self.ref_passive_field = np.random.rand(n_samples, n_samples, n_samples).astype(
            real_t
        )
        self.ref_forcing_field = np.random.rand(n_samples, n_samples, n_samples).astype(
            real_t
        )
    ...
    def check_equals(self, new_passive_field):
        np.testing.assert_allclose(
            self.ref_new_passive_field[1:-1, 1:-1, 1:-1],
            new_passive_field[1:-1, 1:-1, 1:-1],
            atol=self.test_tol,
        )
