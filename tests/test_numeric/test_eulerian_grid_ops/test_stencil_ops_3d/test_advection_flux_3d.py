...
def advection_flux_conservative_eno3_reference(
    field, velocity_x, velocity_y, velocity_z, inv_dx, real_t
):
    kernel_w = 2
...
class AdvectionSolution:
    def __init__(self, n_samples, flux_type="conservative_eno3", precision="single"):
        ...
        if self.flux_type == "conservative_eno3":
            self.kernel_width = 2
            self.ref_advection_flux = advection_flux_conservative_eno3_reference(
                self.ref_field,
                self.ref_velocity_x,
                self.ref_velocity_y,
                self.ref_velocity_z,
