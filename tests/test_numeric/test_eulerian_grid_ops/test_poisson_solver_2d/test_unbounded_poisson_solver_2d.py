...
class UnboundedPoissonSolverSolution2D:
    ...
    def poisson_solve_reference(self):
        ...
        self.domain_doubled_rhs_field[: self.grid_size_y, : self.grid_size_x] = self.rhs_field
        self.domain_doubled_fourier_buffer[...] = rfftn(self.domain_doubled_rhs_field)
        # Greens function convolution
        self.domain_doubled_fourier_buffer[...] *= self.fourier_greens_function_field * (self.dx**2)
        return irfftn(self.domain_doubled_fourier_buffer)[: self.grid_size_y, : self.grid_size_x]
...
def test_unbounded_poisson_solve_neumann_fast_diag_2d(n_values, precision):
    ...
    rhs_field = np.cos(wave_number * np.pi * x_grid) * np.cos(wave_number * np.pi * y_grid)
