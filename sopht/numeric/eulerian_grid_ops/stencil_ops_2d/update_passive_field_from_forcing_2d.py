"""Kernels for updating passive field based on ibm forcing in 2D."""
import numpy as np
...
def gen_update_passive_field_from_forcing_pyst_kernel_2d(
    real_t: type,
    num_threads: bool | int = False,
    fixed_grid_size: tuple[int, int] | bool = False,
) -> Callable:
    # TODO expand docs
    """Update passive field based on feedback forcing in 2D kernel generator."""
    ...
    _update_passive_field_from_forcing_pyst_kernel_2d = ps.create_kernel(
        _update_passive_field_from_forcing_stencil_2d, config=kernel_config
    ).compile()

    def update_passive_field_from_forcing_pyst_kernel_2d(
        ...
        Assumes forcing_field is (n, n).
        """
        _update_passive_field_from_forcing_pyst_kernel_2d(
            passive_field=passive_field,
            forcing_field=forcing_field,
            prefactor=prefactor,
        )

    return update_passive_field_from_forcing_pyst_kernel_2d
